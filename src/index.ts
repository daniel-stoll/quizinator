...
app.post("/store", async (req: express.Request<{}, { id: string } | { error: string }, Quiz>, res) => {
  if (req.body === undefined || Object.keys(req.body).length === 0) {
    res.status(400).json({ error: "Request body must be a non-empty JSON object" });
    return;
  }
  if (!req.body.id) {
  ...
  await fs.writeFile(path.join(DATA_DIR, `${id}.json`), JSON.stringify(req.body));
  res.status(201).json({ id });
});

// GET /store — list all stored JSON objects
app.get("/store", async (_req: express.Request, res: express.Response<Quiz[]>) => {
  await fs.mkdir(DATA_DIR, { recursive: true });
...
app.get("/store/:id", async (req: express.Request<{ id: string }>, res: express.Response<Quiz | { error: string }>) => {
